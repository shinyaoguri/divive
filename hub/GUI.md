...
70.9 : 79.0 : 44.1mmを保ち、上面視は後方が太く前方へ細まる卵形、側面視は下端が
最も太い円錐台です。上面の暗いplate、外周の赤外sensor窪み、前面の状態LED、背面の
USB端子、底面のマウント台座を持たせ、どの向きから見ても前後・上下を判断できます。
本体色と前面LEDは実効tracking stateの色に追従します。実寸のままではpreview内で
見分けられないため、表示上は姿勢を読める大きさへ拡大します。
