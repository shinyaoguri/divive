...
選択中Trackerについて、GUIの10Hz更新時に最新姿勢を最大61点だけ保持し、直近6秒の
位置推移を表示します。BridgeやHubの姿勢処理とは別の表示用履歴であり、過去フレームを
実時間配信へ戻したり、90HzデータをGUIへキューイングしたりしません。

グラフはX、Y、Zをsegmented controlで切り替え、一度に一軸だけを表示します。現在値は
3軸とも常に数値で確認できます。追跡喪失中のsampleは赤い点を併記し、色だけでなく既存の
追跡状態ラベルと組み合わせて判別できるようにします。Sourceを開始または切り替えた時点で
履歴をresetし、異なる座標空間やsessionを一本の線で接続しません。9台以上ではgraphの
高さも抑え、16台の一覧、選択詳細、診断を1120×700pt内へ維持します。
